...
lvm_active=$(lsblk | grep 'lvm' | awk '{if ($1) {printf "yes";exit} else {print "no";exit;}}')
n_active_connect=$(ss -t | grep ESTAB | wc -l)
n_users_server=$(who | cut -d " " -f 1 | sort -u | wc -l)l
...
wall "        #Architecture: $arch
        ...
        #vCPU: $virtproc
        #Memory Free: ${ram_free}/${ram_total}MB ($ram_usage_percent%)
        #Disk Free: ${free_disk}/${total_disk}Gb ($disk_usage_percent%)
