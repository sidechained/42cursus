...
int	main(int argc, char *argv[], char *env[])
{
	...
	while (argv[i] && argv[i + 1]) //check if the end is reached
	{
		...
		while (argv[i] && strcmp(argv[i], ";") && strcmp(argv[i], "|")) // when not
			i++;
		...
		else if (i != 0 && (argv[i] == NULL || strcmp(argv[i], ";") == 0)) //exec in stdout
		{
			...
			else
			{
				...
				while(waitpid(-1, NULL, WUNTRACED) != -1)
					;
				tmp_fd = dup(STDIN_FILENO); // next cmd will take input from stdin
			}
		}
		else if(i != 0 && strcmp(argv[i], "|") == 0) //pipe
		{
			...
			if (fork() == 0)
			{
				dup2(fd[1], STDOUT_FILENO); // output to output end of pipe
				close(fd[0]);
				close(fd[1]);
				if (ft_execute(argv, i, tmp_fd, env))
					return (1);
			...
			else
			{
				close(fd[1]);
				close(tmp_fd);
				tmp_fd = fd[0]; // next cmd will take input from pipe
			}
		}
	}
	close(tmp_fd);
